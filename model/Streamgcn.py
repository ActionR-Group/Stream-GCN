...
class StreamGC(nn.Module):
    def __init__(self, in_channels, out_channels, rel_reduction=8, mid_reduction=1):
        super(StreamGC, self).__init__()
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                bn_init(m, 1)
        self.eca = eca_layer(self.rel_channels, 1)
...
class unit_gcn(nn.Module):  # spatial modeling
    def __init__(self, in_channels, out_channels, A, adaptive=True, residual=True):
        ...
        for i in range(self.num_subset):
            self.convs.append(StreamGC(in_channels, out_channels))
...
class TCN_GCN_unit(nn.Module):  # basic block of Stream-GCN
...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        p_y_given_z = self.classify_z(z_given_v)

        return output, encoder_Z_distr, prior_Z_distr, p_y_given_z
